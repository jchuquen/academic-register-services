package co.grow.plan.academic.register.application.academicplan.subject.ports.spi;

import co.grow.plan.academic.register.domain.academicplan.subject.model.Subject;
import co.grow.plan.academic.register.shared.application.generics.services.IBasicRepositoryForBasicEntity;

public interface ISubjectRepositorySPI
    extends IBasicRepositoryForBasicEntity<Subject> {
}
